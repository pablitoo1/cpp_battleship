...
void Ship3::addField(int field)
{
    fields.push_back(field);
}

void Ship3::deleteField()
{
    fields.clear();
}

bool Ship3::isHit(int field) const
{
...
int Ship3::numberOfFields() const{
    return fields.size();
}

int Ship3::getField(int field) const{
    return fields[field];
}

vector <int> Ship3::getFields() const{
    return fields;
}

bool Ship3::isFieldsOK(HandleGame& handleGame, Menu& menu) const{
    if (fields.size() < 3) {
        return false; // Za ma³o pól, aby spe³niæ warunek
    }

    vector<bool> visited(3, false); // Lista odwiedzonych pól
